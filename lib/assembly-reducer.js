...
const reducer_assembly_path = "src" + path.sep + "data" + path.sep + "assembly-reducer.js"; //装配路径常量
...
const wrapper_handle = (p) => {
    ...
    if(_file_name_ === "interface.js" && _store_name_.endsWith("store")){
        ...
        let relative_path = "";
        if(_common_index_ > 0){// 追加_common
            relative_path = "_common" + p.substring(_common_index_ + common_cond.length).replace(/\\/g,"/").replace(/\.(jsx|js)/,"");
        }
        if(_page_index > 0){// 追加_page
            relative_path = "_page" + p.substring(_page_index + page_cond.length).replace(/\\/g,"/").replace(/\.(jsx|js)/,"");
        }
...
const auto_conf_reducer = (root = 'src') => {
    ...
    let new_conf = "";
    for(var i = 0; i < results_import.length;i++){
        new_conf += (results_import[i] + "\n") ;
    }

    new_conf += "export default combineReducers(" + JSON.stringify(results_reducer).replace(/"/g,"") + ");";
